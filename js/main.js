...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (currentHash === '#archive') {
        ...
        // Check if ID exists, then pass the ID instead of index
        const postExists = ARCHIVE_DATA.some(item => item.id === postId);
        if (postExists) {
            openWritingsModal();
            readArticle(postId); 
        } else {
            openWritingsModal(); // Fallback to grid
        }
    }
...
function openWritingsModal() {
    ...
    // 1. We must define the headerContainer here so JavaScript knows where to put the dropdown
    const headerContainer = document.getElementById('archive-header-container'); 
    ...
    const uniqueYears = [...new Set(ARCHIVE_DATA.map(item => item.created.split('-')[0]))].sort((a, b) => b - a);
    const yearOptions = uniqueYears.map(year => `<option value="${year}">${year}</option>`).join('');
    
    // Inject the Header and BOTH Dropdowns
    headerContainer.innerHTML = `
        <h2>The Notebook Archive</h2>
        <div class="archive-sort-wrapper">
            <select id="archive-year-filter" class="archive-sort-select" onchange="renderArchiveGrid()">
                <option value="all">Year</option>
                ${yearOptions}
            </select>
            <select id="archive-sort" class="archive-sort-select" onchange="renderArchiveGrid()">
    ...
    // 3. Instead of building the grid here, we call the new dedicated sorting function
    renderArchiveGrid();
}
...
function renderArchiveGrid() {
    ...
    const yearSelect = document.getElementById('archive-year-filter');

    const sortValue = sortSelect ? sortSelect.value : 'updated-desc';
    const yearValue = yearSelect ? yearSelect.value : 'all';

    const [sortField, sortDirection] = sortValue.split('-');

    // --- NEW: Filter data by Year first ---
    let filteredData = ARCHIVE_DATA.filter(item => {
        if (yearValue === 'all') return true;
        const itemYear = item.created.split('-')[0]; // Extract the YYYY part
        return itemYear === yearValue;
    });

    // Sort the newly filtered data
    ...
    // Failsafe: Just in case a specific filter combination is empty
    if (filteredData.length === 0) {
        grid.innerHTML = `<p style="grid-column: 1 / -1; color: var(--text-light); text-align: center; padding: 40px;">No entries found for this selection.</p>`;
        return;
    }
...
function readArticle(id) {
    ...
    // Find the exact item by its unique ID
    const item = ARCHIVE_DATA.find(i => i.id === id); 
    
    if (!grid || !item) return;
    if (modal) modal.scrollTop = 0;

    // Replace Dropdown with Breadcrumb header
    headerContainer.innerHTML = `<h2><span onclick="openWritingsModal()" class="breadcrumb-back">Archive</span> / ${toUpperNoAccents(item.title)}</h2>`;
    
    grid.style.display = 'block';
    grid.innerHTML = `
        <div class="markdown-reader-body">
            <div class="reader-meta-group">
                <div class="reader-meta">Published: ${item.created.split('T')[0]}</div>
                <div class="reader-meta">Updated: ${item.date.split('T')[0]}</div>
            </div>
